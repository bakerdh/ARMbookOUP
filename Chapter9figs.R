...
startvals <- c(1,2,40)
sout <- nelder_mead(fitexp, startvals)
p1 <- sout$xmin
...
# this section of code originally used the fminsearch implementation from the neldermead package. This allowed the user access to all vertices of the simplex. However it has now been discontinued and removed from CRAN. So in the interests of continuity I have exported the values into an external file to create this plot.
...
# allxyvals <- array(0,dim=c(7,3,2))
load('data/simplexvals.RData')
...
for (n in 1:5){
  ...
  # sout <- neldermead::fminsearch(fitline, startvals, opt)
  # xyvals <- sout$simplexopt$x
  # allxyvals[n+2,,] <- xyvals
  xyvals <- allxyvals[n+2,,]
  polygon(xyvals[,1],xyvals[,2],col=pal2tone[1])
}
...
# save(file='data/simplexvals.RData',list='allxyvals')
...
sout <- nelder_mead(errorfit, c(1,1))  # fit the model to the data
p <- sout$xmin    # extract the parameter estimates
